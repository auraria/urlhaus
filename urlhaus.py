...
class urlhaus:
    def __init__(self, argv): ##init which assigned the output from req to ip
        ...
        for i in self.ip: #loop to pull ip arguments
            ...
            urls.append(self.url) #appends urlhaus url with ips to search to the urls list
       

    def get(self):
        for u in urls: #loop to go through urls in the urls list
            response = requests.get(u, verify=False) #get request to completed urlhaus url from above
            ...
            results2 = re.findall(r"(?<=\>)http.*(?=\<\/a\>\<\/td\>\<td\>\<span class\=\"badge badge-.*\"\>)", str(soup), re.M|re.I) #regex find all non-html data we want to view
            #print(str(soup))
            print(str(results2))
